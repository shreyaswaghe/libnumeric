...
struct GaussLegendre {
   protected:
	...
	constexpr static T wt4[2] = {0.6521451548625461, 0.3478548451374538};
	constexpr static T ab4[2] = {0.3399810435848563, 0.8611363115940526};

	constexpr static T wt8[4] = {0.3626837833783620, 0.3137066458778873,
								 0.2223810344533745, 0.1012285362903763};
	constexpr static T ab8[4] = {0.1834346424956498, 0.5255324099163290,
								 0.7966664774136267, 0.9602898564975363};

	constexpr static T wt16[8] = {0.1894506104550685, 0.1826034150449236,
								  0.1691565193950025, 0.1495959888165767,
								  0.1246289712555339, 0.0951585116824928,
								  0.0622535239386479, 0.0271524594117541};
	constexpr static T ab16[8] = {0.0950125098376374, 0.2816035507792589,
								  0.4580167776572274, 0.6178762444026438,
								  0.7554044083550030, 0.8656312023878318,
								  0.9445750230732326, 0.9894009349916499};

	constexpr static T wt32[16] = {
   ...
   public:
	...
	T eval(const Func& I) {
		...
		if constexpr (points == 2) {
			return wt2[0] * I.eval(ab2[0]) + wt2[0] * I.eval(-ab2[0]);
		} else if constexpr (points == 4) {
			return wt4[0] * I.eval(ab4[0]) + wt4[0] * I.eval(-ab4[0]) +
				   wt4[1] * I.eval(ab4[1]) + wt4[1] * I.eval(-ab4[1]);
		} else if constexpr (points == 8) {
			return wt8[0] * I.eval(ab8[0]) + wt8[0] * I.eval(-ab8[0]) +
				   wt8[1] * I.eval(ab8[1]) + wt8[1] * I.eval(-ab8[1]) +
				   wt8[2] * I.eval(ab8[2]) + wt8[2] * I.eval(-ab8[2]) +
				   wt8[3] * I.eval(ab8[3]) + wt8[3] * I.eval(-ab8[3]);
		} else if constexpr (points == 16) {
			T sum = 0.0, comp = 0.0, t = 0.0, y = 0.0;
			constexpr uint8_t intPoints = points / 2;
			for (uint8_t i = 0; i < intPoints; i++) {
				y = wt16[i] * I.eval(ab16[i]) - comp;
				t = sum + y;
				comp = (t - sum) - y;
				sum = t;
			...
			return sum;
		} else if constexpr (points == 32) {
			T sum = 0.0, comp = 0.0, t = 0.0, y = 0.0;
			constexpr uint8_t intPoints = points / 2;
			for (uint8_t i = 0; i < intPoints; i++) {
				y = wt32[i] * I.eval(ab32[i]) - comp;
				t = sum + y;
				comp = (t - sum) - y;
				sum = t;
			...
			return sum;
		} else if constexpr (points == 64) {
			constexpr uint8_t intPoints = points / 2;
			T sum = 0.0, comp = 0.0, t = 0.0, y = 0.0;
			for (uint8_t i = 0; i < intPoints; i++) {
				y = wt64[i] * I.eval(ab64[i]) - comp;
				t = sum + y;
				comp = (t - sum) - y;
				sum = t;
